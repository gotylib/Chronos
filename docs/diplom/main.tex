% !TeX program = xelatex
\documentclass[a4paper,14pt]{extarticle}

\usepackage{geometry}
% Поля по методичке (как в требованиях к оформлению ВКР)
\geometry{left=30mm,right=15mm,top=20mm,bottom=20mm}

\usepackage[russian]{babel}
\addto\captionsrussian{%
  \renewcommand{\contentsname}{Содержание}%
  \renewcommand{\figurename}{Рис.}%
}

\usepackage{fontspec}
\setmainfont{Times New Roman}
\setmonofont{DejaVu Sans Mono}[Scale=MatchLowercase]

\usepackage{setspace}
\onehalfspacing

\usepackage{titlesec}
% Заголовок раздела: с новой страницы, по центру, полужирный (14 кегль задаёт класс)
\titleformat{\section}{\clearpage\centering\bfseries}{\thesection}{0.75em}{}
\titlespacing*{\section}{0pt}{3.5ex plus 1ex minus .2ex}{2.3ex plus .2ex}
\titlespacing*{\subsection}{0pt}{2.5ex plus 1ex minus .2ex}{1.5ex plus .2ex}

\setlength{\parindent}{1.25cm}

\usepackage{microtype}
\usepackage{enumitem}
\usepackage{graphicx}
\graphicspath{{фото/}}
\usepackage{amsmath}
\usepackage{array}
\usepackage{longtable}
\usepackage{booktabs}
\usepackage{indentfirst}
\usepackage{xurl}

\usepackage{caption}
% Подпись рисунка: «Рис. 3.1. Название» (нумерация в пределах раздела)
\counterwithin{figure}{section}
\captionsetup[figure]{position=bottom,skip=10pt,labelfont=bf,labelsep=period,%
  justification=centering,font=small}
\captionsetup[table]{position=top,skip=8pt,labelfont=bf}

\usepackage{listings}
\lstdefinestyle{code}{%
  basicstyle=\ttfamily\small,
  columns=fullflexible,
  keepspaces=true,
  breaklines=true,
  frame=single,
  frameround=tttt,
  xleftmargin=0.6em,
  xrightmargin=0.4em,
  tabsize=2,
  showstringspaces=false,
}

\usepackage{tikz}
\usetikzlibrary{arrows.meta,positioning,fit,calc}

\usepackage{hyperref}
\hypersetup{colorlinks=true,linkcolor=black,urlcolor=blue,citecolor=black}
\usepackage{bookmark}

\usepackage[backend=biber,style=numeric,sorting=none]{biblatex}
\addbibresource{references.bib}

\setcounter{secnumdepth}{3}
\setcounter{tocdepth}{3}
% смягчает вылеты длинных слов/путей у правого поля
\emergencystretch=2.5em
\tolerance=2500

\begin{document}

\begin{titlepage}
\thispagestyle{empty}
\begin{center}
{\small
Министерство науки и высшего образования Российской Федерации\\[0.4em]
\textbf{НАЗВАНИЕ УНИВЕРСИТЕТА}\\[0.2em]
Институт / факультет\\
Кафедра \underline{\hspace{6.5cm}}\\[2.2em]
}

{\large\textbf{ВЫПУСКНАЯ КВАЛИФИКАЦИОННАЯ РАБОТА}}\\[0.6em]
{\normalsize по направлению подготовки \underline{\hspace{4cm}}\\[0.3em]
(наименование направления / специальности)}\\[2.5em]

{\large\textbf{Разработка программного комплекса для централизованного\\[0.35em]
управления мультисервисными Docker Compose-проектами\\[0.35em]
(платформа Chronos)}}\\[2.8em]

\begin{flushright}
\begin{minipage}{0.58\linewidth}
\raggedleft
Выполнил:\\
студент группы \underline{\hspace{2.5cm}}\\[0.35em]
\underline{\makebox[\linewidth][l]{}}\\[-0.15em]
{\footnotesize (подпись, дата) \hfill (Ф.\,И.\,О.)}\\[1.1em]
Руководитель:\\[0.35em]
\underline{\makebox[\linewidth][l]{}}\\[-0.15em]
{\footnotesize (подпись, дата) \hfill (Ф.\,И.\,О., учёная степень, звание)}
\end{minipage}
\end{flushright}

\vfill
{\large\textbf{Город} \quad 2026}
\end{center}
\end{titlepage}
\clearpage

\pagenumbering{roman}
\setcounter{page}{2}
\tableofcontents
\clearpage

\pagenumbering{arabic}
\setcounter{page}{1}

\input{chapters/01-intro}
\input{chapters/02-analysis}
\input{chapters/03-design}
\input{chapters/04-implementation}
\input{chapters/05-evaluation}
\input{chapters/06-conclusion}

\clearpage
\printbibliography[title={Список использованных источников}]

\appendix
\input{appendix/appendix-a}
\input{appendix/appendix-b}

\end{document}
