...
chronos_step "Остановка и отключение systemd-сервиса $SERVICE_NAME"
...
chronos_ok "сервис снят"

chronos_step "Удаление $INSTALL_DIR и $ENV_DIR"
...
chronos_ok "каталоги приложения и конфига удалены"

if [[ "$KEEP_DATA" != true ]]; then
  chronos_step "Пользователь $AGENT_USER и $DATA_DIR"
  ...
  chronos_ok "данные и пользователь обработаны"
else
  chronos_step "Режим --keep-data"
  chronos_ok "каталог данных и пользователь сохранены"
fi

echo ""
echo "  [OK] uninstall-chronos-agent.sh завершён успешно."
echo
