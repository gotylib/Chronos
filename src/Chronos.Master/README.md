...
**Центральный сервис**: PostgreSQL для состояния кластера (агенты, аудит, при необходимости маршрутизация), REST API для операций и для встроенного UI, опциональная запись конфигурации **Traefik**, фоновые задачи (лидерство, репликация volumes, очистка).
...
| `Infrastructure/Proxy` | Интеграция с Traefik (файловая динамическая конфигурация). |
